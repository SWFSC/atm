...
# filename <- here::here("data/cal_example_EK80.xml")
# filename <- here::here("data/cal_example_ER60.txt")
...
#              sA = SaValue,
#              outlier = IsSuspended) %>%
#       dplyr::mutate(txdr_freq = txdr.freq,
...
#     # cal.filename <- paste(here("Output/cal", vessel.name), "/", id, ".xml", sep = "")
#     #
...
#
#     # # create calibration file name (results from LOBE, SWING, etc.)
#     # cal.filename <- paste(here("Output/cal", h), "/", id, ".txt", sep = "")
# #
# #     # write calibration file with new filename
...
#   cal.info <- data.frame(id, vessel_name = vessel.name, survey_name = survey.name,
...
#
#   # append cal results
#   cal.res.out   <- dplyr::bind_rows(cal.res.out, cal.res) %>% dplyr::arrange(txdr_freq)
#   # append cal pings
#   cal.pings.out <- dplyr::bind_rows(cal.pings.out, ping.data) %>% dplyr::arrange(txdr_freq, ping_num)
#   # append cal info
#   cal.info.out  <- dplyr::bind_rows(cal.info.out, cal.info) %>% dplyr::arrange(freq)
# }
