...
extract_csv <- function(filename, Sv.max = NULL) {
  ...
  # Rename cps.NASC to cps.nasc, if present
  if ("cps.NASC" %in% colnames(tmp)) {
    dplyr::rename(tmp, cps.nasc = cps.NASC)
  }
  ...
  if ("cps.nasc" %in% colnames(tmp)) {
    tmp %>%
      ...
      dplyr::summarise(
        ...
        dist_m   = Dist_M[1],
        Sv_max   = max(Sv_max),
        NASC.5   = sum(cps.nasc[Layer_depth_max <=   5]),
        NASC.10  = sum(cps.nasc[Layer_depth_max <=  10]),
        NASC.15  = sum(cps.nasc[Layer_depth_max <=  15]),
        NASC.20  = sum(cps.nasc[Layer_depth_max <=  20]),
        NASC.25  = sum(cps.nasc[Layer_depth_max <=  25]),
        NASC.30  = sum(cps.nasc[Layer_depth_max <=  30]),
        NASC.35  = sum(cps.nasc[Layer_depth_max <=  35]),
        NASC.40  = sum(cps.nasc[Layer_depth_max <=  40]),
        NASC.45  = sum(cps.nasc[Layer_depth_max <=  45]),
        NASC.50  = sum(cps.nasc[Layer_depth_max <=  50]),
        NASC.55  = sum(cps.nasc[Layer_depth_max <=  55]),
        NASC.60  = sum(cps.nasc[Layer_depth_max <=  60]),
        NASC.65  = sum(cps.nasc[Layer_depth_max <=  65]),
        NASC.70  = sum(cps.nasc[Layer_depth_max <=  70]),
        NASC.75  = sum(cps.nasc[Layer_depth_max <=  75]),
        NASC.80  = sum(cps.nasc[Layer_depth_max <=  80]),
        NASC.85  = sum(cps.nasc[Layer_depth_max <=  85]),
        NASC.90  = sum(cps.nasc[Layer_depth_max <=  90]),
        NASC.95  = sum(cps.nasc[Layer_depth_max <=  95]),
        NASC.100 = sum(cps.nasc[Layer_depth_max <= 100]),
        NASC.150 = sum(cps.nasc[Layer_depth_max <= 150]),
        NASC.250 = sum(cps.nasc[Layer_depth_max <= 250]),
        NASC.350 = sum(cps.nasc[Layer_depth_max <= 350]),
        NASC     = NASC.350,
